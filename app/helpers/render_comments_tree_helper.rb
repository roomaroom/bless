...
module RenderCommentsTreeHelper
  module Render 
    ...
    class << self
        ...
        "<li>
          <div id='comment_#{@comment.anchor}' class='comment #{@comment.state}' data-comment-id='#{@comment.to_param}'>
            <div>
              ...
              <div >#{ @comment.count_likes }</div>
              #{ h.link_to 'Like', like_comment_path(@comment) }
              #{ h.link_to 'Unlike', unlike_comment_path(@comment) }
              #{ reply }
            </div>
          </div>
      ...
      def avatar
        "<div class='userpic'>
          <img src='#{ @comment.user.profile.avatar_url }' alt='userpic' />
          #{ controls }
        </div>"
      end
